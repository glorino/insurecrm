...
async function processIncomingWhatsApp(from: string, text: string) {
  ...
  let ticketNumber = `INS-${Date.now().toString().slice(-6)}`;
...
• Your order number (starts with # or INS-)
...
We'd love to show you how InsureCRM can transform your customer support, ${customerName}!
...
Have a wonderful day! ✨`;
  ...
  if (fuzzyMatch(lower, ["hello", "hi", "hey", "good morning", "good afternoon", "good evening", "helo", "heloo", "hii", "heythere"])) {
    return `Hello ${customerName}! 👋 Welcome to InsureCRM Support.
